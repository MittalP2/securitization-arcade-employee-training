...
export default function Home(){
  ...
  async function generateCoachDebrief(feedback:DayFeedback){
    ...
    const quizEvidence=activeQuiz.map((item,index)=>({question:item.q,selected:typeof activeAnswers[index]==='number'?item.options[activeAnswers[index]]:'No answer',correct:item.options[item.answer],isCorrect:activeAnswers[index]===item.answer}));
...
function AgentDayDebrief({day,score,total,mastered,initial,coach,close,save,generateCoach,action}:{day:number;score:number;total:number;mastered:boolean;initial?:DayFeedback;coach?:CoachDebrief;close:()=>void;save:(feedback:DayFeedback)=>void;generateCoach:(feedback:DayFeedback)=>Promise<CoachDebrief>;action:(next:'lesson'|'cards'|'quiz'|'close'|'next')=>void}){
  ...
  const [stage,setStage]=useState<'form'|'loading'|'result'>(coach?'result':'form');
  const [result,setResult]=useState<CoachDebrief|undefined>(coach);
  ...
  async function submit(){
    ...
    setStage('loading');
    try{setResult(await generateCoach(feedback));setStage('result')}catch{setStage('result')}
  }
  ...
  {stage==='result'&&result&&<div className="coach-result"><div className="coach-badge"><span>✦</span><p><b>YOUR DAY {day} COACHING PLAN</b>Based on today’s answers and your reflection.</p></div><h2>{result.actionLabel}</h2><p className="coach-summary">{result.summary}</p><div className="coach-insights"><section><span>WHAT YOU UNDERSTOOD</span>{result.mastered.map(item=><p key={item}>✓ {item}</p>)}</section><section><span>WHAT TO PRACTISE</span>{result.growthAreas.map(item=><p key={item}>↺ {item}</p>)}</section></div><div className="coach-recommendation"><span>YOUR NEXT STEP</span><b>{result.actionReason}</b><p><strong>Quick challenge:</strong> {result.challenge}</p></div>{result.reviewQueue.length>0&&<div className="coach-memory"><span>CONCEPTS TO REVISIT</span><p>{result.reviewQueue.join(' · ')}</p></div>}{result.handoff.needed&&<div className="coach-handoff"><span>ASK YOUR TRAINER</span><b>A trainer can help with this question.</b><p>{result.handoff.reason}</p><button onClick={()=>setHandoffOpen(!handoffOpen)}>{handoffOpen?'Hide question':'View question for your trainer'} →</button>{handoffOpen&&<div className="handoff-draft"><p>{result.handoff.draft}</p><button onClick={async()=>{try{await navigator.clipboard.writeText(result.handoff.draft);setCopied(true)}catch{}}}>{copied?'✓ Copied':'Copy question'}</button></div>}</div>}<div className="coach-actions"><button onClick={()=>action(nextAction)}>{result.action==='advance'||result.action==='advance_with_review'?(day<32?`Continue to Day ${day+1}`:'Finish course'):'Follow recommendation'} →</button><button className="edit-feedback" onClick={()=>setStage('form')}>Edit reflection</button></div></div>}
  </section></div>
}
