...
type CoachEvidenceReport = {quiz:{score:number;total:number;missed:Array<{question:string;concept:string}>};reflection:{clarity:string;confidence:string};priorReviewCount:number;reviseTomorrow:Array<{concept:string;reasons:string[]}>};
...
function AgentDayDebrief({day,score,total,mastered,initial,coach,close,save,generateCoach,action}:{day:number;score:number;total:number;mastered:boolean;initial?:DayFeedback;coach?:CoachDebrief;close:()=>void;save:(feedback:DayFeedback)=>void;generateCoach:(feedback:DayFeedback)=>Promise<CoachDebrief>;action:(next:'lesson'|'cards'|'quiz'|'close'|'next')=>void}){
  ...
  const [stage,setStage]=useState<'form'|'loading'|'result'|'error'>(coach?.mode==='fallback'?'error':coach?.evidenceReport?'result':'form');
  const [result,setResult]=useState<CoachDebrief|undefined>(coach?.mode==='agent'&&coach.evidenceReport?coach:undefined);
  ...
  const nextAction=result?.action==='review_prerequisite'?'lesson':result?.action==='retry_practice'?'quiz':result?.action==='advance'||result?.action==='advance_with_review'?day<32?'next':'close':'close';
  return <div className="debrief-layer" role="dialog" aria-modal="true" aria-label={`Day ${day} Learning Coach debrief`}><button className="debrief-backdrop" onClick={close} aria-label="Close Day Debrief"/><section className="day-debrief coach-debrief"><header><div><span className="eyebrow">DAY {day} · LEARNING COACH</span><h2>{stage==='result'&&result?'Your Coach Debrief':'Day Debrief'}</h2><p>{mastered?'Reflect, then let the coach choose your best next move.':'Reflect on the attempt before reviewing the gaps.'}</p></div><button onClick={close}>Close ×</button></header>
  ...
  {stage==='result'&&result&&result.evidenceReport&&<div className="coach-result"><div className="coach-badge"><span>✦</span><p><b>YOUR DAY {day} COACHING PLAN</b>Based on today’s answers and your reflection.</p></div><h2>{result.actionLabel}</h2><p className="coach-summary">{result.summary}</p><div className="coach-strengths"><span>WHAT YOU UNDERSTOOD</span>{result.mastered.map(item=><p key={item}>✓ {item}</p>)}</div><div className="coach-evidence"><span>WHY THIS COACHING PLAN</span><div><p><b>{result.evidenceReport.quiz.score}/{result.evidenceReport.quiz.total}</b>Quiz result</p><p><b>{result.evidenceReport.reflection.clarity.replace('-',' ')}</b>Material clarity</p><p><b>{result.evidenceReport.reflection.confidence.replace('-',' ')}</b>Explain to a colleague</p><p><b>{result.evidenceReport.priorReviewCount}</b>Prior review signals</p></div>{result.evidenceReport.quiz.missed.length>0?<section>{result.evidenceReport.quiz.missed.map(item=><p key={item.question}><b>{item.concept}</b><small>Linked to missed question: “{item.question}”</small></p>)}</section>:<section><p><b>No quiz gaps</b><small>Tomorrow’s concepts are scheduled for retention, not because an answer was wrong.</small></p></section>}</div><div className="coach-recommendation"><span>YOUR NEXT STEP</span><b>{result.actionReason}</b><p><strong>Quick challenge:</strong> {result.challenge}</p></div>{result.handoff.needed&&<div className="coach-handoff"><span>ASK YOUR TRAINER</span><b>A trainer can help with this question.</b><p>{result.handoff.reason}</p><button onClick={()=>setHandoffOpen(!handoffOpen)}>{handoffOpen?'Hide question':'View question for your trainer'} →</button>{handoffOpen&&<div className="handoff-draft"><p>{result.handoff.draft}</p><button onClick={async()=>{try{await navigator.clipboard.writeText(result.handoff.draft);setCopied(true)}catch{}}}>{copied?'✓ Copied':'Copy question'}</button></div>}</div>}<div className="coach-tomorrow"><span>REVISE TOMORROW</span>{result.evidenceReport.reviseTomorrow.map((item,index)=><section key={item.concept}><i>{index+1}</i><p><b>{item.concept}</b><small>{item.reasons.join(' ')}</small></p></section>)}</div><div className="coach-actions"><button onClick={()=>action(nextAction)}>{result.action==='advance'||result.action==='advance_with_review'?(day<32?`Continue to Day ${day+1}`:'Finish course'):'Follow recommendation'} →</button><button className="edit-feedback" onClick={()=>setStage('form')}>Edit reflection</button></div></div>}
  </section></div>
}
