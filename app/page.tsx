  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      <article className="panel lesson-panel" data-tour="lesson">
        ...
        <footer className="lesson-footer"><div><span>DAY {selectedDay} PROGRESS · {activeCompleted}/{requiredSections} SECTIONS</span><div className="footer-meter"><i style={{width:`${activeCompleted/requiredSections*100}%`}} /></div></div><button className="feedback-button" data-tour="feedback" onClick={()=>setFeedbackOpen(true)}>{coachByDay[selectedDay]?'✦ Coach debrief':feedbackByDay[selectedDay]?'✓ Feedback saved':'♡ Give feedback'}</button><button onClick={finishSection}>{section===requiredSections-1?'Finish lesson':'Complete & continue'} <span>→</span></button></footer>
      </article>
...
function AgentDayDebrief({day,score,total,mastered,initial,coach,close,save,generateCoach,action}:{day:number;score:number;total:number;mastered:boolean;initial?:DayFeedback;coach?:CoachDebrief;close:()=>void;save:(feedback:DayFeedback)=>void;generateCoach:(feedback:DayFeedback)=>Promise<CoachDebrief>;action:(next:'lesson'|'cards'|'quiz'|'close')=>void}){
  ...
  const nextAction=result?.action==='review_prerequisite'?'lesson':result?.action==='retry_practice'?'quiz':'close';
