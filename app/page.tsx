...
const tourSteps:TourStep[] = [
  ...
  { target:'[data-tour="feedback"]', eyebrow:'LEARNING COACH', title:'Turn reflection into a personalized next move', text:'Share clarity and confidence after a completed day. The Learning Coach reads the evidence, retrieves approved context, chooses a next action, and saves review concepts in this browser.' },
...
export default function Home(){
  ...
  const completedDays=useMemo(()=>{
    ...
    Object.entries(courseDays).forEach(([key,day])=>{
      ...
      const cardCount=new Set((courseMastered[dayNumber]||[]).filter(index=>index>=0&&index<day.cards.length)).size;
      const dayAnswers=courseAnswers[dayNumber]||{};
      const dayScore=day.quiz.reduce((sum,item,index)=>sum+(dayAnswers[index]===item.answer?1:0),0);
      if(sectionCount===day.sections.length&&cardCount===day.cards.length&&courseSubmitted.includes(dayNumber)&&dayScore/day.quiz.length>=.8)days.add(dayNumber);
    });
    return days;
  ...
  function finishSection(){
    if(progress===100){
      if(selectedDay<32){const masteredDay=selectedDay;chooseLevel(selectedDay);setNotice(`Day ${masteredDay} mastered — welcome to Day ${masteredDay+1}.`)}
      else setNotice('All 32 learning levels mastered — congratulations!');
      return;
    }
    if(selectedDay>1){const done=courseCompleted[selectedDay]||[];if(!done.includes(section))setCourseCompleted({...courseCompleted,[selectedDay]:[...done,section]});if(section<requiredSections-1)setSection(section+1);else{setTool('quiz');setNotice(`Day ${selectedDay} lesson complete — score at least 80% to clear the level.`)}return;}
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      <article className="panel lesson-panel" data-tour="lesson">
        ...
        <footer className="lesson-footer"><div><span>DAY {selectedDay} PROGRESS · {activeCompleted}/{requiredSections} SECTIONS</span><div className="footer-meter"><i style={{width:`${activeCompleted/requiredSections*100}%`}} /></div></div><button className="feedback-button" data-tour="feedback" onClick={()=>setFeedbackOpen(true)}>{coachByDay[selectedDay]?'✦ View Coach Debrief':progress===100?'✦ Generate Coach Debrief':feedbackByDay[selectedDay]?'✓ Feedback saved':'♡ Give feedback'}</button><button onClick={finishSection}>{progress===100?(selectedDay<32?`Continue to Day ${selectedDay+1}`:'Course complete'):section===requiredSections-1?'Finish lesson':'Complete & continue'} <span>→</span></button></footer>
      </article>
...
function AgentDayDebrief({day,score,total,mastered,initial,coach,close,save,generateCoach,action}:{day:number;score:number;total:number;mastered:boolean;initial?:DayFeedback;coach?:CoachDebrief;close:()=>void;save:(feedback:DayFeedback)=>void;generateCoach:(feedback:DayFeedback)=>Promise<CoachDebrief>;action:(next:'lesson'|'cards'|'quiz'|'close'|'next')=>void}){
  ...
  const [stage,setStage]=useState<'form'|'loading'|'result'|'error'>(coach?.mode==='fallback'?'error':coachIsCurrent?'result':'form');
  ...
  async function submit(){
    ...
    save(feedback);
    if(!mastered){setStage('result');return;}
    setStage('loading');
  ...
  const nextAction=day<32?'next':'close';
  ...
  return <div className="debrief-layer" role="dialog" aria-modal="true" aria-label={`Day ${day} Learning Coach debrief`}><button className="debrief-backdrop" onClick={close} aria-label="Close Day Debrief"/><section className="day-debrief coach-debrief"><header><div><span className="eyebrow">DAY {day} · LEARNING COACH</span><h2>{stage==='result'&&result?'Your Quiz Review':'Day Debrief'}</h2><p>{mastered?'See what the quiz revealed and revisit anything you missed.':'Reflect on the attempt before reviewing the gaps.'}</p></div><button onClick={close}>Close ×</button></header>
  {stage==='form'&&<div className="debrief-form"><fieldset><legend>How clear was today’s material?</legend><div className="choice-row">{[['confusing','😕','Confusing'],['some-gaps','🤔','Some gaps'],['clear','💡','Clear']].map(([value,icon,label])=><button type="button" className={clarity===value?'selected':''} onClick={()=>setClarity(value)} key={value}><span>{icon}</span><b>{label}</b></button>)}</div></fieldset><fieldset><legend>Could you explain today’s main concept to a colleague?</legend><div className="choice-row confidence-row">{[['not-yet','Not yet'],['with-help','With help'],['yes','Yes']].map(([value,label])=><button type="button" className={confidence===value?'selected':''} onClick={()=>setConfidence(value)} key={value}>{label}</button>)}</div></fieldset><fieldset><legend>What would improve this lesson? <small>Select any that apply.</small></legend><div className="improvement-grid">{improvementOptions.map(option=><button type="button" className={improvements.includes(option)?'selected':''} onClick={()=>toggle(option)} key={option}>{improvements.includes(option)?'✓ ':''}{option}</button>)}</div></fieldset><label className="comment-field"><b>Anything else?</b><small>Optional · {comment.length}/500</small><textarea maxLength={500} value={comment} onChange={event=>setComment(event.target.value)} placeholder="Tell the coach what worked or where you got stuck…"/></label><div className="debrief-note"><span>ⓘ</span><p><b>Coaching, not surveillance.</b>Your learning signals personalize support and are not a standalone employee-performance score.</p></div><button className="submit-debrief" disabled={!clarity||!confidence} onClick={submit}>{mastered?`Save & generate Day ${day} Coach Debrief →`:`Save feedback & review gaps →`}</button></div>}
  {stage==='loading'&&<div className="coach-loading"><span>✦</span><h2>Your Learning Coach is connecting the dots…</h2><div><p>✓ Reviewing today’s answers</p><p>✓ Connecting your reflection</p><p>○ Choosing your most useful next step</p></div></div>}
  {stage==='error'&&<div className="debrief-result coach-error"><span>!</span><small>COACH DEBRIEF UNAVAILABLE</small><h2>Could not produce results</h2><p>Feedback sent to the app owner. Please try again later.</p><button onClick={()=>setStage('form')}>Try again →</button><button className="edit-feedback" onClick={close}>Close</button></div>}
