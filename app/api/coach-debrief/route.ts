...
const tools:ToolDefinition[]=[
  ...
  {type:'function',function:{name:'choose_learning_action',description:'Choose the single next action that best completes the learning workflow based on submitted quiz evidence.',parameters:{type:'object',properties:{action:{type:'string',enum:['advance','advance_with_review','review_prerequisite','retry_practice','trainer_handoff']},focus_concepts:{type:'array',items:{type:'string'},maxItems:3},reason:{type:'string'}},required:['action','focus_concepts','reason']}}},
...
function buildEvidenceReport(input:AgentInput){
  ...
  return {quiz:{score:input.score,total:input.total,missed},priorReviewCount:input.previousMemory.reduce((sum,item)=>sum+item.reviewQueue.length,0),reviseTomorrow};
}
...
export async function POST(request:Request){
  ...
  if(!input||!Number.isInteger(input.day)||input.day<1||input.day>32||!Number.isInteger(input.score)||!Number.isInteger(input.total)||input.total<1||input.score<0||input.score>input.total||!input.approvedContext||!Array.isArray(input.quizEvidence))return Response.json({error:'Invalid coaching evidence.'},{status:400});
  ...
  const system=`You are Study Arcade’s bounded Learning Coach. Complete an end-of-day learning workflow using approved quiz evidence only. Never invent finance facts, change the syllabus, score employee performance, or send a message. A handoff is appropriate only for unsupported, incorrect, deal-specific, legal, accounting, rating, investment, or source-owner questions. Keep coaching concise, encouraging, and specific. Do not reveal hidden reasoning.`;
  const evidence={day:input.day,score:input.score,total:input.total,xp:input.xp,quizEvidence:input.quizEvidence,previousMemory:input.previousMemory,missedConcept:evidenceReport.quiz.missed[0]||null,availableConceptNames:input.approvedContext.concepts.map(item=>item.term)};
  ...
  try{
    ...
    return Response.json({mode:'agent',summary:safeText(raw.summary,`You completed Day ${input.day} and the coach reviewed your quiz evidence.`),mastered:cleanStrings(raw.mastered,2).length?cleanStrings(raw.mastered,2):[`Day ${input.day} learning loop completed`,`${input.score}/${input.total} quiz accuracy`],growthAreas:cleanStrings(raw.growthAreas,2).length?cleanStrings(raw.growthAreas,2):focusConcepts,action,actionLabel:safeText(raw.actionLabel,action==='advance'?'Ready for the next level':'Continue with targeted review',100),actionReason,challenge:safeText(raw.challenge,`Explain ${focusConcepts[0]||'today’s main concept'} to a colleague without using your notes.`),conceptExample:evidenceReport.quiz.missed.length?safeText(raw.example,`Think through the correct answer using this rule: ${evidenceReport.quiz.missed[0].explanation||evidenceReport.quiz.missed[0].definition}`):'',reviewQueue,evidenceReport,handoff:{needed:handoffNeeded,reason:handoffNeeded?safeText(memory.args.reason,actionReason):'',draft:handoffNeeded?safeText(memory.args.draft,`I completed Day ${input.day} but would like help with ${focusConcepts.join(', ')}.`):''},activity:['Read submitted quiz evidence','Retrieved approved course context','Chose the next learning action',handoffNeeded?'Prepared a trainer handoff for human review':'Prepared the device-local review queue']});
